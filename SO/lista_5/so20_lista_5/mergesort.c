...
static void Sort(int parent_fd) {
  ...
  int left_noe = nelem / 2;
  int right_noe = nelem - left_noe;

  // Wysyłanie elementów do posortowania odpowiednio
  // do lewego i prawego dziecka
  SendElem(parent_fd, left.parent_fd, left_noe);
  SendElem(parent_fd, right.parent_fd, right_noe);

  // Scalanie posortowanych ciągów
  Merge(left.parent_fd, right.parent_fd, parent_fd);

  // Zamknięcie pozostałych gniazd
  Close(left.parent_fd);
  Close(right.parent_fd);
