...
static noreturn void coro_switch(int v)
{
  ...
  running = TAILQ_NEXT(running, co_link);     // wybierz kolejny proces w kolejce

  if (running == TAILQ_END(&runqueue)){       // Gdy kolejny procces = NULL to wybierz pierwszy
    running = TAILQ_FIRST(&runqueue);         // proces z kolejki (zapętlenie kolejki)
  }

  if (v == EOF){                               // Usuń współprogram z kolejki po napotkaniu EOF
    TAILQ_REMOVE(&runqueue, curr, co_link);
  }

  if (TAILQ_EMPTY(&runqueue)){                  // Jeśli kolejka jest już pusta 
    Longjmp(dispatcher, v);                     // powróć do dispachera, bo już wszystkie
  }                                             // współprogramy zakończyły działanie   
