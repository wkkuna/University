...
(struct boolean?-expr   (e)        #:transparent)
(struct number?-expr    (e)        #:transparent)
...
(define (expr? e)
  (match e
    ...
    [(boolean?-expr e) (expr? e)]
    [(number?-expr e)  (expr? e)]
    [(and-expr e1 e2)  (and (expr? e1)
                            (expr? e2))]
    [(or-expr e1 e2)   (and (expr? e1)
                            (expr? e2))]
    [_ false]))
...
(define (parse q)
  (cond
    ...
    [(and (list? q) (eq? (length q) 3) (eq? (first q) 'and))
     (and-expr (parse (second q))
               (parse (third q)))]
    
    [(and (list? q) (eq? (length q) 3) (eq? (first q) 'or))
     (or-expr  (parse (second q))
               (parse (third q)))]
...
(define (eval-env e env)
  (match e
    ...
    [(boolean?-expr e)  (boolean? (eval-env e env))]
    [(number?-expr e)   (number? (eval-env e env))]
    [(and-expr e1 e2)   (and (eval-env e1 env)
                             (eval-env e2 env))]
    [(or-expr e1 e2)    (or  (eval-env e1 env)
                             (eval-env e2 env))]))
