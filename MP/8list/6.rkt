...
(define (parse q)
  (cond
    ...
    [(and (list? q) (eq? (length q) 2) (symbol? (first q))
          (eq? (first (string->list (symbol->string (first q)))) #\c))
     (parse-car-cdr (cdr (string->list (symbol->string (first q)))) (parse (second q)))]
    
    [(and (list? q) (eq? (length q) 2) (not (eq? (op->proc (first q)) false))) ; <!!!
...
(define (parse-car-cdr sym expr)
  (cond [(eq? #\r (car sym))   expr]
        [(eq? #\a (car sym))   (unop 'car (parse-car-cdr (cdr sym) expr))]
        [(eq? #\d (car sym))   (unop 'cdr (parse-car-cdr (cdr sym) expr))]
