...
while True:
    ...
    if game_phase == "RELEASE" and len(moves["RELEASE"]):
        ...
        if moves["TASK_PRIORITIZATION"]:
            ...
            possible_get = [mv[0] for mv in moves["TASK_PRIORITIZATION"]]
            possible_throw = [mv[1] for mv in moves["TASK_PRIORITIZATION"]]
            for ncard in ncards:
                for rcard in rcards:
                    if ncard in possible_get and rcard in possible_throw:
        ...
        if not output and moves["CONTINUOUS_INTEGRATION"]:
            for app in ranked_applications:
                ncards = get_needed_card_list(app, hand, deck_status, applications)
                for ncard in ncards:
                    if ncard in moves["CONTINUOUS_INTEGRATION"]:
                        print("CONTINUOUS_INTEGRATION", ncard)
                        output = True
                        break
                if output:
                    break
