...
class Applications:
    ...
    def rank_applications(self, us, cards):
        self.permute_apps(us.score, cards)
        # print(self.combs, file=sys.stderr, flush=True)
        return self.combs[0][0], self.combs[0][1]
...
class Cards:
    ...
    def get_riddable_card_set(self, costs):
...
while True:
    ...
    # rank applications
    hand = cards.get_hand()
    ranked_applications, costs = applications.rank_applications(us, cards)
    ncards = cards.get_needed_card_list(ranked_applications, us.score)
    rcards = cards.get_riddable_card_set(costs)
