...
def alpha_beta(game, player, depth, legal_moves):
    alpha = ninf
    beta = inf

    _, best_move = alpha_beta_search(
        game, player, depth, alpha, beta, player, legal_moves)
    return best_move


def alpha_beta_search(game, player, depth, alpha, beta, maxing_player, moves):
    if game.is_terminal() != -1 or not depth:
        return (heuristic(game, maxing_player), None)

    best_move = φ
    best_result = ninf if player == maxing_player else inf

    def better(
        x, y): return x > y if player == maxing_player else lambda x, y: x < y
    for move in moves:
        new_game = copy.deepcopy(game)
        new_game.move(move)

        result, _ = alpha_beta_search(
            new_game, opponent(player), depth - 1, alpha, beta, maxing_player, new_game.possible_moves(opponent(player)))
...
while True:
    ...
    best = alpha_beta(game, player, 2, legal_moves)
